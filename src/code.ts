...
figma.ui.onmessage = ({ type, data, chartConfig, fields, dimensions }) => {
  ...
  if (type === 'draw-data') {
    let nodes = []
    if (chartConfig.type == "bars") drawBars({ chartConfig, data, fields, dimensions, nodes })
    if (chartConfig.type == "scatter") drawScatter({ chartConfig, data, fields, dimensions, nodes })
    if (chartConfig.type == "line") drawLine({ chartConfig, data, fields, dimensions, nodes })
...
const drawBars = ({ chartConfig, data, fields, dimensions, nodes }) => {
  const barWidth = Math.max(chartConfig.width / data.length - fields.padding, 3)
...
const drawLine = ({ chartConfig, data, fields, dimensions, nodes }) => {
  ...
  let areaPath = chartConfig.areaPath.replace(/,/g, " ").replace(/([A-Z])/g, " $1 ")
  ...
  let linePath = chartConfig.linePath.replace(/,/g, " ").replace(/([A-Z])/g, " $1 ")
